...
public class Term implements Comparable<Term> {
	...
	public static class PrefixOrder implements Comparator<Term> {
		...
		public int compare(Term v, Term w) {
			...
			int lengthV = v.myWord.length();
			int lengthW = w.myWord.length();
			int min = Math.min(lengthV, lengthW);
			if(min < r) {
				if ((lengthV < lengthW) && (v.myWord.substring(0, min).compareTo(w.myWord.substring(0, min))==0)) {
					return -1;
				}
				if((lengthW < lengthV) && (v.myWord.substring(0, min).compareTo(w.myWord.substring(0, min))==0)) {
					return 1;
				}
			}
			String vFinal = v.myWord.substring(0, Math.min(min, r));
			String wFinal = w.myWord.substring(0, Math.min(min, r));
			return vFinal.compareTo(wFinal);
		}
	}
